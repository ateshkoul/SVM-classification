...
% alternate feature scaling
% The idea was to separately normalise the two objects, however this will
% apply different normalisations to the data and thus is not correct. All
% the features should be scaled on the same scale as they will be entered
% together for the classification.

% WRONG METHOD different scaling to the diff objects:
% for i = 1:size(unique(Data(:,1)),1)
%     p = Data(Data(:,1)==i,2:end);
%     p = (p - repmat(min(p),size(p,1),1))./repmat((max(p)-min(p)),size(p,1),1);
%     Data(Data(:,1)==i,2:end) = p;
% end

% Feature scaling to interval [0,1]
% p = Data(:,2:end);
% p = (p - repmat(min(p),size(p,1),1))./repmat((max(p)-min(p)),size(p,1),1);
% Data(:,2:end) = p;
...
% Standarization
p = Data(:,2:end);
p = (p - repmat(mean(p),size(p,1),1))./repmat(std(p),size(p,1),1);
Data(:,2:end) = p;
...
for folds=1:nfold
    ...
    for i = 1:numLog2c
        ...
        for j = 1:numLog2g
            ...
            param = ['-m 2000 -q -t 2 -v 3 -c ', num2str(2^log2c), ' -g ', num2str(2^log2g)];
    ...
    param = ['-m 2000 -q -t 2 -c ', num2str(2^bestLog2c), ' -g ', num2str(2^bestLog2g), ' -b 1'];
    ...
    acc = libsvmcrossval_ova(testLabel, testData, param, nfoldtrain);
